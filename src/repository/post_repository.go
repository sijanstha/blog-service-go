...
func NewPostRepository(dbPort db.DbPort) IPostRepository {
	return &postRepository{db: dbPort.GetDbConnection()}
}
...
func (p *postRepository) FindAllWithPagination(filter post.PostListFilter) *post.PostPaginationDetails {
	...
	selectStmt, err := p.db.Prepare(selectAllQuery)
	if err != nil {
		logger.Error("invalid query", err)
		return result
	}
	...
	countRowsStmt, err := p.db.Prepare(countRowsQuery)
	if err != nil {
		logger.Error("invalid query", err)
		return result
	}
