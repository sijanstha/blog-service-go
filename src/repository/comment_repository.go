...
func NewCommentRepository(dbPort db.DbPort) ICommentRepository {
	return &commentRepository{db: dbPort.GetDbConnection()}
}
...
func (c *commentRepository) FindAllWithPagination(filter comment.CommentListFilter) *comment.CommentPaginationDetails {
	...
	selectStmt, err := c.db.Prepare(selectAllQuery)
	if err != nil {
		logger.Error("invalid query", err)
		return result
	}
	...
	countRowsStmt, err := c.db.Prepare(countRowsQuery)
	if err != nil {
		logger.Error("invalid query", err)
		return result
	}
