...
func NewUserRepository(dbPort db.DbPort) IUserRepository {
	return &userRepository{db: dbPort.GetDbConnection()}
}
...
func (repo *userRepository) FindAllWithPagination(filter user.UserListFilter) *user.UserPaginationDetails {
	...
	selectStmt, err := repo.db.Prepare(selectAllQuery)
	if err != nil {
		logger.Error("invalid query", err)
		return result
	}
	...
	countRowsStmt, err := repo.db.Prepare(countRowsQuery)
	if err != nil {
		logger.Error("invalid query", err)
		return result
	}
